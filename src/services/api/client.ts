...
async function request<T>(
  ...
  try {
    ...
    if (!res.ok) {
      ...
      const errPayload = data as unknown as { message?: string; detail?: string } | null
      const message = (errPayload && (errPayload.detail || errPayload.message))
        ? (errPayload.detail || errPayload.message)!
